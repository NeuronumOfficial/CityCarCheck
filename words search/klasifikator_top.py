...
def search_and_label(dirpath: str,
    ...
    for fp in _iter_paths(dirpath, extensions):
        ...
        score = score_matches_for_file(raw, keywords, fuzzy_threshold, use_fuzzy)
...
def load_keywords_from_file(path: str) -> List[str]:
    ...
    with open(path, "r", encoding="utf-8") as f:
        lines = [normalize_text(line.strip()) for line in f if line.strip()]
    print(f"Loaded {len(lines)} keywords from {path}")
    return lines
...
def main():
    ap = argparse.ArgumentParser(description="Klasifikator s citanim klucovvch slov z .txt suboru.")
    ap.add_argument("--dir", required=True, help="Directory or single file to scan")
    ap.add_argument("--keywords", default="", help="Comma-separated keywords")
    ap.add_argument("--keywords-file", help="Path to .txt file with one keyword per line")
    ...
    ap.add_argument("--min-score", type=int, default=2, help="Min hits for approval")
